...
def readSQLcase():
    ...
    coon = pymysql.connect(user='testdb',passwd='test123456',db='autotest',port=3306,host='127.0.0.1',charset='utf8')
    cursor = coon.cursor()
...
def interfaceTest(case_list):
    ...
    for case in case_list:
        ...
        if method.upper() == 'GET':
            headers = {'Authorization':'','Conten-Type':'applcation/json'}
        ...
        if method.upper()=='PUT':
            ...
            if 'pass' == res:
                ...
                writeBug(case_id,interface_name,new_url,results,res_check) #P86
            try:
                preOrderSN(results)
            except:
                print('ok')
        if method.upper()=="PATCH":
           headers = {'Authorization':'Credential '+id,'Content-Type':'applcation/json' }
           data = None
...
def CredentialId():
    ...
    headers = { 'Connection':'keep-alive','Content-Type':'applicaion/json'}
...
def writeResult(case_id,result):
    ...
    sql = "UPDATE apitest_apistep set apitest_apistep.apistatus=%s where apitest_apistep.Apistest_id=%s;"
...
def writeBug(bug_id,interface_name,request,response,res_check):
    ...
    "VALUES ('%s','%s','1','1','aaa','aaa','%s','2');"%(bugname,pymysql.escape_string(bugdetail),now)
    coon = pymysql.connect(user='testdb',passwd='test123456',db='autotest',port=3306,host='127.0.0.1',charset='utf8')
    cursor = coon.cursor()
    cursor.execute(sql)
    coon.commit()
